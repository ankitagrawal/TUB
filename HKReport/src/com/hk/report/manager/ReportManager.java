...
public class ReportManager {
  ...
  */
   ...
   */
  public File generateCourierReportXsl(String xslFilePath, EnumShippingOrderStatus shippingOrderStatus, List<Courier> courierList,Date startDate,Date endDate, Warehouse warehouse) throws Exception {
      ...
      if (shippingOrderList != null) {
          for (ShippingOrder order : shippingOrderList) {
              ...
              Category diabetes = getCategoryService().getCategoryByName(CategoryConstants.DIABETES);
              Category eye = getCategoryService().getCategoryByName(CategoryConstants.EYE);
  ...
  public File generateCourierReportXslForBlueDart(String xslFilePath, EnumShippingOrderStatus orderStatus, List<Courier> courierList, Date startDate, Date endDate, Warehouse warehouse) throws Exception {
    List<ShippingOrder> shippingOrderList=null;
    File file = new File(xslFilePath);
    FileOutputStream out = new FileOutputStream(file);
    Workbook wb = new HSSFWorkbook();
    ...
    setCellValue(row, 0, ReportConstants.AWB);
    setCellValue(row, 1, ReportConstants.SHIPPERS_REFERENCE_NUMBER);
    setCellValue(row, 2, ReportConstants.SHIPPER);
    setCellValue(row, 3, ReportConstants.CONSIGNEE_NAME);
    setCellValue(row, 4, ReportConstants.CONSIGNEE_ADDRESS_LINE_1);
    setCellValue(row, 5, ReportConstants.CONSIGNEE_ADDRESS_LINE_2);
    setCellValue(row, 6, ReportConstants.CONSIGNEE_ADDRESS_LINE_3);
    setCellValue(row, 7, ReportConstants.CONSIGNEE_PINCODE);
    setCellValue(row, 8, ReportConstants.CONSIGNEE_PHONE_NUMBER);
    setCellValue(row, 9, ReportConstants.CONSIGNEE_MOBILE);
    setCellValue(row, 10, ReportConstants.PRODUCT);
    setCellValue(row, 11, ReportConstants.CONTENT_DESCRIPTION);
    setCellValue(row, 12, ReportConstants.WEIGHT);
    setCellValue(row, 13, ReportConstants.DECLARED_VALUE);
    setCellValue(row, 14, ReportConstants.COD_AMOUNT);
    setCellValue(row, 15, ReportConstants.SHIPMENT_TYPE);
...
//            }
  ...
  public List<CODConfirmationDto> generateUnConfirmedOrderList(Date startDate, Date endDate) {
    ...
    for (CODConfirmationDto codConfirmationDto : codUnConfirmationDtos) {
      computeActualTimeTakenToConfirm(codConfirmationDto, ReportConstants.permissibleHourCount, ReportConstants.dayBeginHour,
          ReportConstants.dayEndHour, ReportConstants.COD_UNCONFIRMED);
    }
  ...
  /*    public List<GoogleBannedWordDto> generateDailyGoogleAdsBannedWords() {

          List<GoogleBannedWordDto> googleBannedWordDtoList = new ArrayList<GoogleBannedWordDto>();
          googleBannedWordDtoList = googleBannedWordDaoProvider.get().getGoogleBannedWordDtoList();

...
public class BusyPopulateRtoData {
  ...
  public void transactionHeaderForRtoGenerator() {
    ...
    sql.eachRow("""
									...
									w.name as warehouse, w.id as warehouse_id, so.amount as net_amount, c.name as courier_name,if(so.drop_shipping =1,'DropShip',if(so.is_service_order =1,'Services',if(bo.is_b2b_order=1,'B2B','B2C'))) Order_type,

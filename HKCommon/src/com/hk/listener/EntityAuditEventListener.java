...
public class EntityAuditEventListener implements PostUpdateEventListener, PostDeleteEventListener, PostInsertEventListener{
    ...
    public void onPostUpdate(PostUpdateEvent postUpdateEvent) {
      ...
      Long entityId = null;
      try {
        entityId = Long.parseLong(postUpdateEvent.getId().toString());
      } catch (NumberFormatException nfe) {
        logger.info("No auditing done for entityId" + postUpdateEvent.getId().toString());
      }
    ...
    public void onPostInsert(PostInsertEvent postInsertEvent) {
      ...
      Long entityId = null;
      try {
        entityId = Long.parseLong(postInsertEvent.getId().toString());
      } catch (NumberFormatException nfe) {
        logger.info("No auditing done for entityId" + postInsertEvent.getId().toString());
      }

      if (entityId != null && postInsertEvent.getEntity() != null && isAuditable(postInsertEvent.getEntity().getClass())) {
        getAuditService().audit(AuditAction.INSERT, postInsertEvent.getEntity(), entityId, postInsertEvent.getState(),
            new Object[postInsertEvent.getState().length], postInsertEvent.getPersister().getPropertyNames(), propertyTypeNames);

        /*logger.info(entityId + " : " + postInsertEvent.getState() + " : " +
            new Object[postInsertEvent.getState().length] + " : " + postInsertEvent.getPersister().getPropertyNames() + " : " + propertyTypeNames);*/
      }
    }
    ...
    public void onPostDelete(PostDeleteEvent postDeleteEvent) {
      ...
      if (postDeleteEvent.getEntity() != null && isAuditable(postDeleteEvent.getEntity().getClass())) {
        getAuditService().audit(AuditAction.DELETE, postDeleteEvent.getEntity(), Long.parseLong(postDeleteEvent.getId().toString()),

...
public class SessionStoreInterceptor implements Interceptor {
    ...
    public Resolution intercept(ExecutionContext context) throws Exception {
      ...
      // Closing hbm session.
      if (LifecycleStage.RequestComplete.equals(context.getLifecycleStage())) {
       /* try {
          org.hibernate.Session session = ((Provider<org.hibernate.Session>) BaseDao.staticSessionProvider.get()).get();
          logger.debug("session = "+session);
          session.clear();
          //session.close();
        } catch (Exception e) {
          //logger.error("Exception while closing hbm session", e);
        }*/
      }

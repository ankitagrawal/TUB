...
<s:layout-component name="htmlHead">
	...
	<script type="text/javascript">
        ...
        taxOptions += '<option value="'+${tax.id}+'">'+${tax.value}+'</option>';
        </c:forEach>
				 </c:when>
				<c:otherwise>
        ...
        taxOptions += '<option value="'+${surcharge.id}+'">' + ${surcharge.value} + '</option>';
        </c:forEach>
				</c:otherwise>
				</c:choose>
...
<s:layout-component name="content">
	<div style="display: none;">
		<s:link beanclass="com.hk.web.action.admin.inventory.PurchaseInvoiceAction" id="pvInfoLink" event="getPVDetails"></s:link>
	</div>
	...
	<s:form beanclass="com.hk.web.action.admin.inventory.PurchaseInvoiceAction">
		...
		<table border="1">
			...
			<c:forEach var="purchaseInvoiceLineItem" items="${pia.purchaseInvoice.purchaseInvoiceLineItems}" varStatus="ctr">
				...
				<s:hidden name="purchaseInvoiceLineItems[${ctr.index}].productVariant" value="${purchaseInvoiceLineItem.sku.productVariant.id}"/>
				...
				<tr count="${ctr.index}" class="${ctr.last ? 'lastRow lineItemRow':'lineItemRow'}">
					...
					<td class="taxCategory">
						...
						<c:otherwise>
							 <s:select name="purchaseInvoiceLineItems[${ctr.index}].surcharge"
								...
								<hk:master-data-collection service="<%=MasterDataDao.class%>" serviceProperty="surchargeList" value="id"
								                           label="value"/>
							</s:select>
						</c:otherwise>
						</c:choose>
						...
						<shiro:lacksPermission name="<%=PermissionConstants.UPDATE_RECONCILIATION_REPORTS%>">
							<c:choose>
								<c:when test="${pia.purchaseInvoice.supplier.state==pia.purchaseInvoice.warehouse.state}">
							<s:text name="purchaseInvoiceLineItems[${ctr.index}].tax" value="${purchaseInvoiceLineItem.tax.value}" class="taxCategory"/>
							</c:when>
								<c:otherwise>
									 <s:text name="purchaseInvoiceLineItems[${ctr.index}].surcharge" value="${purchaseInvoiceLineItem.surcharge.value}" class="taxCategory"/>
								</c:otherwise>
								</c:choose>
					...
					<td>
						<s:text name="purchaseInvoiceLineItems[${ctr.index}].costPrice" value="${purchaseInvoiceLineItem.costPrice}"
						        class="costPrice valueChange"/>
					</td>
					<td>
						<s:text class="mrp" name="purchaseInvoiceLineItems[${ctr.index}].mrp" value="${purchaseInvoiceLineItem.mrp}"/>
					</td>
					<td>
						<s:text class="discountPercentage valueChange" name="purchaseInvoiceLineItems[${ctr.index}].discountPercent" value="${purchaseInvoiceLineItem.discountPercent}"/>
					</td>
					<td>
						<s:text readonly="readonly" class="taxableAmount" name="purchaseInvoiceLineItems[${ctr.index}].taxableAmount" value="${purchaseInvoiceLineItem.taxableAmount}"/>
					</td>
					<td>
						<s:text readonly="readonly" class="taxAmount" name="purchaseInvoiceLineItems[${ctr.index}].taxAmount" value="${purchaseInvoiceLineItem.taxAmount}"/>
					</td>
						<td>
        ...
        <td colspan="4" class="totalQuantity"></td>
				...
				<td><s:text readonly="readonly" class="totalPayable" name="purchaseInvoice.payableAmount" value="${pia.purchaseInvoice.payableAmount}"/></td>
			</tr>
			<tr>
				<td colspan="12"></td><td>Overall Discount<br/>(In Rupees)</td>
				<td><s:text class="overallDiscount footerChanges" name="purchaseInvoice.discount" value="${pia.purchaseInvoice.discount}"/></td>
			</tr>
			<tr>
					<tr>
				<td colspan="12"></td><td>Freight and Forwarding<br/>Charges(In Rupees)</td>
				<td><s:text class="freightCharges footerChanges" name="purchaseInvoice.freightForwardingCharges" value="${pia.purchaseInvoice.freightForwardingCharges}"/></td>
			</tr>
			<tr>
				 <td colspan="12"></td><td>Final Payable</td>
					<td><s:text readonly="readonly" class="finalPayable" name="purchaseInvoice.finalPayableAmount" value="${pia.purchaseInvoice.finalPayableAmount}"/></td>
			</tr>
			</tfoot>
		</table>

    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('.rto').live("change", function() {
                ...
                var recheckinOk = shippingRow.find('.ok-qty').val();
                var recheckinDamaged = shippingRow.find('.damaged-qty').val();
                var recheckinExpired = shippingRow.find('.expired-qty').val();
                var sumOfReturned = recheckinOk + recheckinDamaged + recheckinExpired;
                var qty = shippingRow.find('.qty').html();
                if (Math.round(sumOfReturned) > Math.round(qty)) {
...
<s:layout-component name="content">
    ...
    <c:if test="${orderAdmin.shippingOrder != null}">
        <s:form beanclass="com.hk.web.action.admin.inventory.SearchOrderAndReCheckinRTOInventoryAction"
                                    ...
                                    <c:forEach items="${orderAdmin.shippingOrder.lineItems}" var="lineItem"
                                        ...
                                        <c:if test="${!productVariant.product.service}">
                                            ...
                                            <tr class="shippingRow">
                                                ...
                                                <td title="Re-checkin">
                                                    <s:text name="lineItemRecheckinQtyMap[${lineItem}]" size="1"
                                                            style="width:25px;" class="rto re-checkin" value="0"/>
                                                </td>
                                                <td>
            ...
            <div class="buttons"><s:submit name="checkinReturnedUnits" value="Checkin Returned Units"/></div>
        </s:form>

...
<s:useActionBean beanclass="web.action.admin.order.search.SearchOrderAction" var="orderAdmin" event="pre"/>
...
<s:errors/>
<s:form beanclass="web.action.admin.order.search.SearchOrderAction" method="get" autocomplete="false">
...
<td>
  ...
  <c:if test="${order.orderStatus.id == orderStatusPending || order.orderStatus.id == orderStatusHold}">
    <s:link beanclass="web.action.admin.order.OrderOnHoldAction" event="unHoldOrder" title="Unhold Order"
    ...
    <s:link beanclass="web.action.admin.order.OrderOnHoldAction" event="holdOrder" title="Put Order on Hold"
  ...
  <c:if test="${!(order.orderStatus.id == orderStatusCancelled || order.orderStatus.id == orderStatusCart)}">
    <br/>
    <s:form beanclass="web.action.admin.order.CancelOrderAction" class="cancelOrderForm">
      <s:param name="order" value="${order.id}"/>
      Reason:
...
<td>
  ...
  <c:if test="${order.orderStatus.id != orderStatusCart}">
    ...
    <%--<br/>(<s:link beanclass="web.action.AccountingInvoiceAction" event="oldAccountingInvoice" target="_blank">--%>
    ...
    <%--<s:link beanclass="web.action.AccountingInvoiceAction" event="retailInvoice" target="_blank">--%>
    ...
    <%--<s:link beanclass="web.action.AccountingInvoiceAction" event="b2bInvoice" target="_blank">--%>
    <%--<s:param name="accountingInvoice" value="${accountingInvoice.id}"/>--%>
    ...
    <%--<s:link beanclass="web.action.AccountingInvoiceAction" event="serviceInvoice" target="_blank">--%>
    <%--<s:param name="accountingInvoice" value="${accountingInvoice.id}"/>--%>
  ...
  <c:choose>
    ...
    <c:otherwise>
      ...
      ${order.user.email}
        ...
        <span class="sml">
        (<s:link beanclass="web.action.admin.SearchTicketAction" event="search" target="_blank">
          View Tickets
  ...
  <i><fmt:formatDate value="${order.payment.createDate}" type="both"/></i> <br/>
      <span class="sml">
        <s:link beanclass="web.action.admin.CreateTicketAction" event="createOrderRelatedTicket">
          Create Ticket
          <s:param name="order" value="${order.id}"/>
        </s:link> <br/>
        <s:link beanclass="web.action.admin.SearchTicketAction" event="search" target="_blank">
  ...
  <table>
    ...
    <tr class="addressRow">
      <td width="150">
        ...
        <br/>
        <s:form beanclass="web.action.HomeAction" autocomplete="false">
        ...
        <s:link beanclass="web.action.admin.SetDefaultCourierAction" event="pre" class="setAsDefaultCourierLink">
          Set as default
    ...
    <c:if test="${order.orderStatus.id == orderStatusPending || order.orderStatus.id == orderStatusHold}">
      <tr>
        <td><s:link beanclass="web.action.admin.ChangeOrderAddressAction" event="pre">
...
<td class="has_table">
  ...
  <s:link beanclass="web.action.admin.order.OrderCommentAction" event="pre" target="_blank">
    ...
    <table>
      <c:forEach items="${order.shippingOrders}" var="shippingOrder">
        <tr>
          <td>
            GatewayId:
            <s:link beanclass="web.action.admin.order.search.SearchShippingOrderAction"
            ...
            <c:if test="${shippingOrder.shipment !=null}">
              Track Link: <s:link beanclass="web.action.TrackCourierAction" target="_blank">

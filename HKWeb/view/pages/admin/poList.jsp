...
<s:layout-render name="/layouts/defaultAdmin.jsp" pageTitle="Purchase Order List">
  <s:useActionBean beanclass="web.action.admin.POAction" var="poa"/>
  ...
  <s:layout-component name="content">
    ...
    <fieldset class="right_label">
      ...
      <s:form beanclass="web.action.admin.POAction">
    ...
    <table class="zebra_vert">
      ...
      <c:forEach items="${poa.purchaseOrderList}" var="purchaseOrder" varStatus="ctr">
        <tr>
          ...
          <td>
            <s:link beanclass="web.action.admin.EditPurchaseOrderAction">Edit
              <s:param name="purchaseOrder" value="${purchaseOrder.id}"/></s:link>
            &nbsp;
            <s:link beanclass="web.action.admin.POAction" event="print" target="_blank">Print
              <s:param name="purchaseOrder" value="${purchaseOrder.id}"/></s:link>
            &nbsp;
            <s:link beanclass="web.action.admin.POAction" event="poInExcel" target="_blank">Excel
              <s:param name="purchaseOrder" value="${purchaseOrder.id}"/></s:link>
            <c:if test="${purchaseOrder.purchaseOrderStatus.id >= approved}">
                <br/>
                <s:link beanclass="web.action.admin.POAction" event="generateGRNCheck">Create GRN
                  <s:param name="purchaseOrder" value="${purchaseOrder.id}"/></s:link>
            </c:if>
          </td>
        </tr>

...
<s:layout-render name="/layouts/checkoutLayout.jsp" pageTitle="Payment Options">
...
<div class='pre'>
  <h4>
    Your total billable amount is
    <strong class='num'>
      <fmt:formatNumber value="${orderSummary.pricingDto.grandTotalPayable}" type="currency" currencySymbol="Rs "/>
    </strong>
  </h4>
  <h6>
    If you have any trouble during the payment process, call our helpline number
    <strong class='red'>
      0124 - 4551616
    </strong>
...
<div class="alert messages" style="font-size:14px; color:red">
  <s:errors/><s:messages key="generalMessages"/>
</div>
...
<div style="visibility:hidden;" class="offer-banner">
...
<div id="tabs_content1" class="tab_content">
  <s:form beanclass="com.hk.web.action.payment.PaymentAction" method="post">
    ...
    <p>
      <label><s:radio name="bankId" value="70"/>VISA &nbsp;</label>
      <img src="<hk:vhostImage/>/images/visa.jpg" height="20px">
    </p>

    <p>
      <label><s:radio name="bankId" value="670"/> VISA/MasterCard/Maestro &nbsp;</label>
      <img src="<hk:vhostImage/>/images/mastercard.jpg" height="30px">
    </p>

    <div style="float: right; width: 90%;">
      <s:submit name="proceed" value="Make Payment >" class="button" disabled="${fn:length(orderSummary.pricingDto.outOfStockLineItems) > 0 ? 'true':'false'}"/>
    </div>
  </s:form>
</div>
<div id="tabs_content2" class="tab_content" style="display:none;">
  <s:form beanclass="com.hk.web.action.payment.PaymentAction" method="post">
    ...
    <div style="float: right; width: 90%;">
      <s:submit name="proceed" value="Make Payment >" class="button makePayment" disabled="${fn:length(orderSummary.pricingDto.outOfStockLineItems) > 0 ? 'true':'false'}"/>
    </div>
  </s:form>
</div>
<div id="tabs_content3" class="tab_content" style="display:none;">
  <s:form beanclass="com.hk.web.action.payment.PaymentAction" method="post">
    ...
    <div style="float:left;  margin-left: 20px; line-height: 21px;">
      <div class="brandBox">
        <table width="100%">
          <c:forEach items="${paymentModeBean.bankList}" var="bank" varStatus="idx">
            <c:if test="${idx.index%2 == 0}">
              <tr>
            </c:if>
            <td class="col">
              <s:radio name="bankId" value="${bank.id}"/>${bank.bankName}
            </td>
    ...
    <c:choose>
      <c:when test="${orderSummary.codAllowed}">
        ...
        <div class="grid_5">
          ...
          <p><strong><u>Order Total</u></strong>
            <fmt:formatNumber value="${orderSummary.pricingDto.grandTotalPayable}" currencySymbol="Rs. "
                              type="currency"/><br/>
            <u>COD Charges</u>
            <fmt:formatNumber value="${orderSummary.codCharges}" currencySymbol="Rs. " type="currency"/>
            <br/>
            <strong class="orange"><u>Grand Total</u></strong>
            <strong class="orange"><fmt:formatNumber
        ...
        <s:form beanclass="com.hk.web.action.payment.CodPaymentReceiveAction" method="post">
          ...
          <s:text name="codContactPhone" value="${orderSummary.order.address.phone}"/>
          <div class="buttons" style="font-size: 1.3em;">
            <br/><br/>
            <s:submit name="pre" value="Place Order" class="positive makePayment"/></div>
          <br/><br/>
          ...
          <p>
            Please ensure that the above person is available at the given location for pick-up at all
            times.<br/>
          </p>
        </s:form>
      </c:when>
      <c:otherwise>
        ...
        <p>
          Cash on Delivery is not available for your delivery location (Pincode :
          <strong>${orderSummary.order.address.pin}</strong>). <br/>
          We provide cash on delivery option for select PIN codes only.
        </p>

        <p><strong>OR</strong></p>

        <p>The net payable is not in the range of <strong>Rs. ${codMinAmount} - Rs. ${codMaxAmount}</strong></p>
      </c:otherwise>
    </c:choose>
  </div>
  <div id="tabs_content5" class="tab_content" style="display:none;">
    ...
    <div class="left" style="padding-left: 20px;">
      <s:form beanclass="com.hk.web.action.payment.ChequeCashPaymentReceiveAction" method="post" id="paymentForm">
        ...
        <label><s:radio name='paymentMode' value='<%=EnumPaymentMode.NEFT.getId()%>' checked="checked"/>&nbsp;Online
          Transfer(NEFT)</label><br/>
        <label><s:radio name='paymentMode' value='<%=EnumPaymentMode.CashDeposit.getId()%>'/>&nbsp;Cash
          Deposit</label><br/>
        <label><s:radio name='paymentMode' value='<%=EnumPaymentMode.ChequeDeposit.getId()%>'/>&nbsp;Cheque</label><br/>
        ...
        <div class="num" style="font-size: 1.8em; font-weight: bold; color: #F87500;">
          <fmt:formatNumber value="${orderSummary.pricingDto.grandTotalPayable}" currencySymbol="Rs. " type="currency"/>
        </div>
        <div style="width: 50%; float: right;">
          <s:submit name="pre" value="Place Order" class="button makePayment" style="font-size: 1.5em;"/>
        </div>
      </s:form>
    </div>
    <div class="right" style="width: 30%; padding: 10px; line-height: 21px;">
      <h5>HealthKart Bank Account
        Details</h5>
      <br/>
      <strong>Name of A/C</strong>
      <br/>Aquamarine HealthCare Pvt. Ltd.
      <br/><br/>
      <strong>Bank</strong>
      <br/>ICICI Bank, Dwarka Sector-5 Branch, New Delhi
      <br/><br/>
      <strong>Type</strong>
      <br/>Current Account
      <br/><br/>
      ...
      <br/>025005003746
      <br/><br/>
      <strong>RTGS/NEFT/IFSC Code</strong>
      <br/>ICIC0000250
      <br/><br/>
      <br/>
    </div>

...
<s:layout-render name="/layouts/productLayout.jsp"
...
.rating_bar {
	width: 80px;
	background:
		url('${pageContext.request.contextPath}/images/img/star-off.png') 0 0
		repeat-x;
}
...
.rating_bar div {
	height: 16px;
	background:
		url('${pageContext.request.contextPath}/images/img/star-on.png') 0 0
		repeat-x;
}
...
.frameSize td {
	...
	padding: 5px;
		...
		<script type="text/javascript"
			src="<hk:vhostJs/>/js/jquery.jqzoom-core.js"></script>
	...
	<s:layout-component name="modal">
		...
		<c:if test="${!empty product.productVariants[0].productExtraOptions}">
			<s:layout-render name="/pages/modal/productWithExtraOptions.jsp"
				product="${product}" />
		</c:if>
		...
		<c:if test="${pa.combo == null}">
			<shiro:hasPermission name="<%=PermissionConstants.GET_PRODUCT_LINK%>">
				<s:layout-render name="/pages/modal/productLink.jsp"
					product="${product}" affiliateId="${pa.affiliate.id}" />

				<s:layout-render name="/pages/modal/productBannerLink.jsp"
					product="${product}" affiliateId="${pa.affiliate.id}" />
			</shiro:hasPermission>
		</c:if>
	...
	<s:layout-component name="breadcrumbs">
		<s:layout-render name="/layouts/embed/catalogBreadcrumb.jsp"
			breadcrumbProduct="${product}" lastLink="true"
	...
	<s:layout-component name="topBanner">
		<s:layout-render name="/layouts/embed/_categoryTopBanners.jsp"
			topCategories="${pa.topCategoryUrlSlug}" />
		<div class="clear"></div>
		<c:if test="${product.service}">
			<s:layout-render name="/layouts/embed/changePreferredZone.jsp"
				filterUrlFragment="${pa.urlFragment}" />
		</c:if>
	...
	<s:layout-component name="prod_title">
		...
		<div><shiro:hasPermission
			name="<%=PermissionConstants.UPDATE_SEO_METADATA%>">
			<s:link beanclass="com.hk.web.action.core.content.seo.SeoAction"
				event="pre" target="_blank" class="popup">Edit MetaData
        <s:param name="seoData" value="${seoData.id}" />
			</s:link>
      &nbsp;|&nbsp;
    </shiro:hasPermission> <shiro:hasPermission
			name="<%=PermissionConstants.UPDATE_PRODUCT_CATALOG%>">
			<s:link
				beanclass="com.hk.web.action.admin.catalog.product.EditProductAttributesAction"
				event="editProductVariantDetails" target="_blank" class="popup">
        ...
        <s:param name="product" value="${product}" />
			...
			<s:link beanclass="com.hk.web.action.admin.sku.SkuAction"
				event="searchSKUs" target="_blank" class="popup">
        Edit Sku Attributes
	...
	<s:layout-component name="prod_slideshow">
		...
		<div class="img320"><a
			href="${hk:getS3ImageUrl(imageLargeSize, product.mainImageId,isSecure)}"
			class="jqzoom" rel='gal1' title="${product.name}"> <img
			src="${hk:getS3ImageUrl(imageMediumSize, product.mainImageId,isSecure)}"
			alt="${product.name}" title="${product.name}"> </a></div>
		<div><c:if test="${fn:length(pa.productImages) > 1}">
			<ul class="thumblist">
		...
		<div class="clear"></div>
		<div style="padding-top: 15px"><shiro:hasPermission
			name="<%=PermissionConstants.GET_PRODUCT_LINK%>">
			<a name="showProductLink" class="linkbutton"
				onclick="$('#getProductLinkWindow').jqm(); $('#getProductLinkWindow').jqmShow();"
				style="cursor: pointer">Get Links</a>
			<a name="showProductLink" class="linkbutton"
				onclick="$('#getBannerLinkWindow').jqm(); $('#getBannerLinkWindow').jqmShow();"
				style="cursor: pointer">Get Banners</a>
		</shiro:hasPermission></div>
		<div class="clear"></div>
		<div><shiro:hasPermission
			name="<%=PermissionConstants.MANAGE_IMAGE%>">
			<s:link
				beanclass="com.hk.web.action.core.catalog.image.UploadImageAction"
				event="pre" target="_blank" class="popup"> Upload
                <s:param name="product" value="${product.id}" />
			</s:link>
			<s:link
				beanclass="com.hk.web.action.admin.catalog.product.EditProductAttributesAction"
				event="manageProductImages" target="_blank" class="popup">Manage
                Images
                <s:param name="productId" value="${product.id}" />
			</s:link>
		</shiro:hasPermission></div>
		<c:if test="${product.videoEmbedCode != null}">
			<s:layout-render name="/layouts/embed/_productVideo.jsp"
	...
	<s:layout-component name="product_detail_links">
		...
		<div class='infos' style="border-bottom: 0px;"><c:if
			test="${hk:isNotBlank(product.brand)}">
			<span class='title'> Brand: </span>
			<span class='info'> <s:link
				beanclass="com.hk.web.action.core.catalog.BrandCatalogAction"
				class="bl">
              ${product.brand}
              <s:param name="brand"
					value="${fn:toLowerCase(product.brand)}" />
				<s:param name="topLevelCategory" value="${pa.topCategoryUrlSlug}" />
			</s:link> </span>
		</c:if> <c:if test="${hk:isNotBlank(product.manufacturer)}">
      |
          <span class='title'> Manufacturer: </span>
			<span class='info'> ${product.manufacturer.name} </span>
		</c:if> | <span class='title'> Dispatched in: </span> <span
			class='info orange'
			title="Delivery time is extra depending on the location"> <c:choose>
		...
		<div class='top_links'><c:if
			test="${!empty product.description}">
			<a class='top_link' href='#description'> Description &darr; </a>
		</c:if> <c:if test="${!empty product.productFeatures}">
			<a class='top_link' href='#features'> Technical Specs &darr; </a>
		</c:if> <c:if test="${!empty pa.relatedCombos}">
			<a class='top_link' href='#related_combos' style="font-weight: bold;">
			Special Offers &darr; </a>
		</c:if> <c:if test="${!empty product.relatedProducts}">
			<a class='top_link' href='#related_products'> Related Products
			&darr; </a>
		</c:if></div>
		<c:if test="${hk:isNotBlank(product.overview)}">
			<p class="overview">${product.overview}</p>
		</c:if>
		<input type="hidden" id="productReferrerId"
			value="${pa.productReferrerId}" />
		<shiro:hasPermission
			name="<%=PermissionConstants.UPDATE_PRODUCT_DESCRIPTIONS%>">
			<div><s:link
				beanclass="com.hk.web.action.admin.catalog.product.EditProductAttributesAction"
				event="editOverview" class="popup">
        Edit Overview
        <s:param name="productId" value="${product.id}" />
			</s:link></div>
		</shiro:hasPermission>
	...
	<s:layout-component name="product_variants">
		<c:choose>
			<c:when
				test="${empty product.productVariants[0].productExtraOptions}">
				<c:choose>
					<c:when test="${fn:length(product.productVariants) > 1}">
						<c:choose>
							<c:when test="${!product.productHaveColorOptions}">
								<s:layout-render
									name="/layouts/embed/_productWithMultipleVariantsWithNoColorOptions.jsp"
									product="${product}" />
								<s:layout-render
									name="/layouts/embed/_hkAssistanceMessageForMultiVariants.jsp" />
							</c:when>
							<c:otherwise>
								<s:layout-render
									name="/layouts/embed/_productWithMultipleVariantsWithColorOptions.jsp"
									product="${product}" />
								<s:layout-render
									name="/layouts/embed/_hkAssistanceMessageForMultiVariants.jsp" />
							</c:otherwise>
					...
					<c:otherwise>
						<c:choose>
							<c:when test="${pa.combo != null}">
								<s:layout-render name="/layouts/embed/_comboProduct.jsp"
									productId="${product.id}" />
							</c:when>
							<c:when
								test="${hk:collectionContains(product.categories, eyeGlass)}">
								<s:layout-render name="/layouts/embed/glasses.jsp"
									product="${product}" />
							</c:when>
							<c:otherwise>
								<s:layout-render
									name="/layouts/embed/_productWithSingleVariant.jsp"
									product="${product}" />
							</c:otherwise>
						</c:choose>
					</c:otherwise>
			...
			<c:otherwise>
				<s:layout-render name="/layouts/embed/_productWithExtraOptions.jsp"
					product="${product}" />
		...
		<shiro:hasPermission name="<%=PermissionConstants.MANAGE_IMAGE%>">
			<div><s:link
				beanclass="com.hk.web.action.core.catalog.image.UploadImageAction"
				event="uploadVariantImage" target="_blank" class="popup"> Upload
        <s:param name="productVariant"
					value="${product.productVariants[0]}" />
			</s:link> &nbsp;|&nbsp; <s:link
				beanclass="com.hk.web.action.core.catalog.product.ProductVariantAction"
				event="renderManageImages" target="_blank" class="popup">Manage
        Images
        <s:param name="productVariant"
					value="${product.productVariants[0]}" />
			</s:link></div>
	...
	<s:layout-component name="product_description">
		...
		<c:if test="${!empty pa.relatedCombos}">
			...
			<h4>Special Offers on ${product.name}</h4>
			<c:forEach items="${pa.relatedCombos}" var="relatedCombo">
				<s:layout-render name="/layouts/embed/_productThumb.jsp"
					productId="${relatedCombo.id}" />
			</c:forEach>

			<div class="floatfix"></div>
			<a class='go_to_top' href='#top'>go to top &uarr;</a></div>
		</c:if>
		<c:if test="${hk:collectionContains(product.categories, eyeGlass)}">
				...
				<table width="900px;" style="margin-left: 10px; margin-right: 10px;">
					<tr>
						<c:forEach items="${product.variations}" var="variation">
							...
							<a style="text-decoration: none; cursor: pointer; border-bottom: none;">
							 <hk:productImage imageId="${variation.productVariation.mainImageId}"
                                                                     size="<%=EnumImageSize.SmallSize%>"
                                                                     alt="${variation.productVariation.name}"/>
							
							<p class="productOptions">${variation.relationShip}</p>
							</a></div>
							</td>
			...
			<div id="sizeGuide" class="content"
				style="background-color: #DDDDDD; padding: 5px; cursor: pointer; font-weight: bold; text-align: left;">
			Size Guide</div>
			<div id="frameChart">
			<table width="900px;">
				<tr>
					<td><img
						src="${pageContext.request.contextPath}/images/banners/frame_chart.jpg" />
					</td>
					<td valign="top" class="content"
						style="vertical-align: top; padding-top: 10px;">It is very
					important to verify the size of your eyes before you venture into
					the market looking for the perfect pair of frames to wear. This
					manual will help you decide what the pre-requisites are and how to
					go about it in a proper structured manner.<a target="_blank"
		...
		<c:if
			test="${pa.addressDistanceDtos != null && fn:length(pa.addressDistanceDtos) > 0}">
			<h3>Service Centres Available Near to You : <br>
			<ul>
				<c:forEach items="${pa.addressDistanceDtos}" var="addressDto">
					<li>${addressDto.localityMap.address.line1},
					${addressDto.localityMap.address.line2}<br>
					</li>
				</c:forEach>
			</ul>
			</h3>
		</c:if>
		<p><a class='go_to_top' href='#top' style="float: right;">go
		to top &uarr;</a></p>
		...
		<shiro:hasPermission
			...
			<div><s:link
				beanclass="com.hk.web.action.admin.catalog.product.EditProductAttributesAction"
				event="editDescription" class="popup">
        Edit Description
        <s:param name="productId" value="${product.id}" />
			</s:link></div>
		</shiro:hasPermission>
		...
		<shiro:hasPermission
			...
			<s:link
				beanclass="com.hk.web.action.admin.catalog.product.EditProductAttributesAction"
				event="editFeatures" class="popup">
      Edit Features
      <s:param name="product" value="${product}" />
			</s:link>
		</shiro:hasPermission>
	...
	<s:layout-component name="related_products">
		...
		<c:if test="${!empty relatedProducts}">
			...
			<h4>People who bought this also bought these products</h4>

			<c:forEach items="${relatedProducts}" var="relatedProduct">
				<s:layout-render name="/layouts/embed/_productThumbG.jsp"
	...
	<s:layout-component name="user_reviews">
		...
		<c:choose>
			<c:when test="${!empty pa.userReviews}">
				<table width="950" class="reviewLinksTable">
					<tr height="40">
						<td style="font-size: 14px; font-weight: bold; border-style: none">Reviews
						of ${product.name}</td>
						<td style="border-style: none"><s:link
					...
					<tr height="50">
						<td colspan="2" style="border-style: none">Average Rating : <strong><fmt:formatNumber
							value="${pa.averageRating}" maxFractionDigits="1" />/5</strong> <br />
						(based on ${pa.totalReviews} reviews) <br />

						<div class="rating_bar">
						<div id="blueStar" class="blueStar"><script
							type="text/javascript">
                          var averageRating = ${pa.averageRating};
                          averageRating = (averageRating * 20) + "%";
				...
				<c:forEach items="${pa.userReviews}" var="review" varStatus="ctr">
					...
					<table width="950" class="reviewContentTable"
                  ...
                  $('.blueStarRating' + index).width(rating);
							...
							<div style="word-wrap: break-word">${review.review}</div>
							</td>
						</tr>
						<tr style="border-style: none">
		...
		<c:otherwise>
			<table width="950" style="border: none">
				<tr height="40">
					<td colspan="2" style="font-size: 14px; font-weight: bold;">No
					Reviews!!</td>
				</tr>
				<tr>
					<td>Be the first one to <s:link
						beanclass="com.hk.web.action.core.catalog.product.ProductReviewAction"
						event="writeNewReview">
						<s:param name="product" value="${product.id}" />
						<strong>Write a Review</strong>

...
<p style="margin-bottom:1em">
  Thanks for placing an order with us. We have received the order and are waiting for the payment via <strong>${order.payment.paymentMode.name}</strong>.
  Please note that <strong>${order.payment.paymentMode.name}</strong> orders will not be confirmed and shipped from our end until we receive the payment.
</p>
<p>
  If we do not receive the payment in our bank account within the next 3-4 business days, we will assume that you do not want to proceed with the order,
...
<p style="margin-bottom:1em">Here are your order details for the order <strong> ${order.gatewayOrderId}</strong> placed on<strong> ${order.payment.createDate} </strong></p>

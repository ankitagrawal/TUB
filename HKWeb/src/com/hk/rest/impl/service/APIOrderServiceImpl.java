...
public class APIOrderServiceImpl implements APIOrderService {
  ...
  public String createOrderInHK(APIOrder apiOrder) {
    ...
    order.setCartLineItems(new HashSet<CartLineItem>());
    order = getOrderService().save(order);
    //add items in the cart
    addCartLineItems(apiOrder.getApiOrderDetails(), hkUser, order);
  ...
  public Payment createPayment(Order order, APIPayment apiPayment) {
    ...
    return getPaymentManager().createNewPayment(order, paymentMode, "182.12.1.1", apiPayment.getBankId()); //remote ip adddress is hard coded
  }

  public void addCartLineItems(APIOrderDetails apiOrderDetails, User hkUser, Order order) {
    ...
    for (APIProductDetail detail : apiOrderDetails.getApiProductDetails()) {
      ...
      //no need to store history now
      //userCartDao.get().addToCartHistory(productVariant.getProduct(), hkUser);
      //userProductHistoryDao.get().updateIsAddedToCart(productVariant.getProduct(), hkUser);

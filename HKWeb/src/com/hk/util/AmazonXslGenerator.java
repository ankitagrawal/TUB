...
public class AmazonXslGenerator {
      ...
      for (ProductVariant productVariant : product.getProductVariants()) {
        ...
        if (!productVariant.isDeleted()) {
          ...
          if (org.apache.commons.lang.StringUtils.isNotBlank(amazonFeedDbDescription) && amazonFeedDbDescription != null) {
            ...
            setCellValue(row, 10, product.getDescription() != null ? Jsoup.parse(product.getDescription()).text() : product.getOverview() != null ? Jsoup.parse(product.getOverview()).text() : null);
          }
  ...
  public File generateTempExcel(String xslFilePath) throws Exception {
    ...
    List<DoomDay> doomDayList = doomDayDao.listAll();
    ...
    setCellValue(row, 0, "PRIMARY_CATEGORY");
    setCellValue(row, 1, "VARIANT_ID");
    setCellValue(row, 2, "NAME");
    setCellValue(row, 3, "INV_SNAPSHOT");
    setCellValue(row, 4, "CHECKED_IN_INV");
    setCellValue(row, 4, "DIFFERENCE");

    int initialRowNo = 1;
    for (DoomDay doomDay : doomDayList) {
      List<ProductVariant> productVariants = productVariantDao.findVariantFromBarcode(doomDay.getUpcVariantId());
      if (productVariants == null || productVariants.isEmpty() || productVariants.size() > 1) {
        System.out.println("bug   " + doomDay.getUpcVariantId());
//        logger.info("bug   " + upc);
      } else {
        row = sheet1.createRow(initialRowNo);
        for (int columnNo = 0; columnNo < totalColumnNo; columnNo++) {
        ...
        setCellValue(row, 0, productVariants.get(0).getProduct().getPrimaryCategory().getDisplayName());
        setCellValue(row, 1, productVariants.get(0).getId());
        setCellValue(row, 2, productVariants.get(0).getProduct().getName());
        setCellValue(row, 3, productVariants.get(0).getNetInventory());
        setCellValue(row, 4, doomDayDao.getCheckedInQty(doomDay.getUpcVariantId()));
        initialRowNo++;
      }
    }

...
public class LatLongGenerator {
  ...
  public void parseXmlResponse(String city, String state) throws IOException {
    ...
    try {
      HttpURLConnection connection = (HttpURLConnection) new URL("http://maps.googleapis.com/maps/api/geocode/xml?address=" + URLEncoder.encode(city, "UTF-8") + "," + URLEncoder.encode(state, "UTF-8") + ",India&sensor=true&region=in").openConnection();
  ...
  public void createLocalityMap(Address address, Manufacturer manufacturer) {
    ...
    try {
      HttpURLConnection connection = (HttpURLConnection) new URL("http://maps.googleapis.com/maps/api/geocode/xml?address=" + URLEncoder.encode(address.getLine2(), "UTF-8") + "," + URLEncoder.encode(address.getCity(), "UTF-8") + "," + URLEncoder.encode(address.getState(), "UTF-8") + "&sensor=true&region=in").openConnection(); //add ,India

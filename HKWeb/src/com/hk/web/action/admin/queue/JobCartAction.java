    ...
    try {
      if (productVariant != null && skuGroupField != null) {
        ...
        if (bin != null) {
          ...
          if (status) {
            addRedirectAlertMessage(new SimpleMessage("Bin assisgned for" + productVariant.getProduct().getName() + "With Barcode :" + skuGroupField.getBarcode() + " at bin " + bin.getBarcode()));
      ...
      if (listOfSkuGroupListPerGrn.size() > 0) {
        for (List<SkuGroup> grpList : listOfSkuGroupListPerGrn) {
          for (SkuGroup skugroup : grpList) {
            ...
            if (binIds == null) {
              addRedirectAlertMessage(new SimpleMessage("Please Assign Bin to Product Variant  with Batch  : " + skugroup.getBatchNumber() + " having Barcode :  " + skugroup.getBarcode()));
              return new RedirectResolution(JobCartAction.class).addParameter("putList").addParameter("grn", grn.getId());
            }
      ...
      try {
        ...
        for (List<SkuGroup> skugrouplist : listOfSkuGroupListPerGrn) {
          for (SkuGroup skuGroup : skugrouplist) {
            List<SkuItem> skuItemListPerSkuGroup = adminSkuItemDao.getCheckedInSkuItems(skuGroup); //NO. of sku item per sku group

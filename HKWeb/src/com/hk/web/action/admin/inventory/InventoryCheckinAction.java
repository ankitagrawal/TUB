...
public class InventoryCheckinAction extends BaseAction {
  ...
  WarehouseDao warehouseDao;
	...
	private final double TOLERANCE_LEVEL_PERCENTAGE = 10; // Max allowed
		...
		try {
			...
			if (productVariant != null) {
				if (costPrice != null && (costPrice > productVariant.getCostPrice() + TOLERANCE_LEVEL_PERCENTAGE * productVariant.getCostPrice() / 100)) {
					...
					/*
					 * "Cost price is higher than the maximum permissible limit of " +
					 * TOLERANCE_LEVEL_PERCENTAGE + " %. \n" +
					 */
					...
					healthkartResponse = new HealthkartResponse(HealthkartResponse.STATUS_ERROR,
					/*
					 * "Cost price is lesser than the maximum permissible limit of " +
					 * TOLERANCE_LEVEL_PERCENTAGE + " %. \n" +
					 */
					"Cost price of the variant in the system is Rs. " + productVariant.getCostPrice() + "\n Do you want to continue?", dataMap);
					...
					healthkartResponse = new HealthkartResponse(HealthkartResponse.STATUS_ERROR,
					/*
					 * "MRP is higher than the maximum permissible limit of " +
					 * TOLERANCE_LEVEL_PERCENTAGE +" %. \n" +
					 */
					...
					healthkartResponse = new HealthkartResponse(HealthkartResponse.STATUS_ERROR,
					/*
					 * "MRP is lesser than the maximum permissible limit of " +
					 * TOLERANCE_LEVEL_PERCENTAGE +" %. \n" +
					 */
					"MRP of the variant in the system is Rs. " + productVariant.getMarkedPrice() + "\n Do you want to continue?", dataMap);
				} else {
		...
		try {
			...
			if (productVariant != null) {
				...
				if (grnLineItem != null && sku != null) {
					...
					// SkuGroup skuGroup =
					// getAdminInventoryService().createSkuGroup(batch, mfgDate,
					// expiryDate, costPrice, mrp, grn, null, null, sku);
					...
					// getPoLineItemService().updatePoLineItemFillRate(grn, grnLineItem,
					// grnLineItem.getCheckedInQty());
	...
	public Resolution checkinInvBarcode() {
		...
		if (skuItem != null) {
			...
			if (foreignShippingOrderGatewayId != null && foreignShippingOrderGatewayId.equals(grn.getInvoiceNumber())) {
				...
				// Independent Code - Common in else
				getInventoryService().checkInventoryHealth(productVariant);
				if (getInventoryService().allInventoryCheckedIn(grn)) {
				...
				addRedirectAlertMessage(new SimpleMessage("Oops! Trying to checkout SKU Item from different PO/Invoice"));
				return new RedirectResolution(InventoryCheckinAction.class).addParameter("grn", grn.getId());
			}
		} else { // Planned Checking using Barcode Map
				...
				if (productVariant != null) {
                  ...
                  if (shippingOrders != null && shippingOrders.size() > 0) {
                    ...
                    message.concat("<br>Shipping Order(s) - " + escalated + " were escalated.");
									...
									getGoodsReceivedNoteDao().save(grn);
									editPVFillRate(grn);
									message ="All inventory checked in. GRN is marked closed";
	...
	public Resolution saveStockTransfer() {
		...
		if (stockTransferLineItemAgainstCheckInSkuGrp == null) {
			checkinSkuGroup = getAdminInventoryService().createSkuGroupWithoutBarcode(skuGroup.getBatchNumber(), skuGroup.getMfgDate(), skuGroup.getExpiryDate(),
					skuGroup.getCostPrice(), skuGroup.getMrp(), null, null, stockTransfer, sku);
		...
		if (skuItem != null) {
			...
			if (stockTransferLineItem.getCheckedinQty() == null || (!stockTransferLineItem.getCheckedinQty().equals(stockTransferLineItem.getCheckedoutQty()))) {
				...
				// adminInventoryService.inventoryCheckoutForStockTransfer(sku, skuItem,
				// stockTransferLineItem, 1L, loggedOnUser,
				// getInventoryService().getInventoryTxnType(EnumInvTxnType.STOCK_TRANSFER_CHECKIN),
				// EnumSkuItemStatus.Checked_IN.getSkuItemStatus());
				adminInventoryService.inventoryCheckinCheckout(sku, skuItem, null, null, null, null, stockTransferLineItem, EnumSkuItemStatus.Checked_IN,
						EnumSkuItemOwner.SELF, getInventoryService().getInventoryTxnType(EnumInvTxnType.STOCK_TRANSFER_CHECKIN), 1L, loggedOnUser);
		...
		return new RedirectResolution(StockTransferAction.class, "checkinInventoryAgainstStockTransfer").addParameter("stockTransfer", stockTransfer.getId())
				.addParameter("messageColor", "green");
	}
	...
	public Resolution downloadAllBarcode() {
		...
		for (GrnLineItem grnLineItem : grnLineItems) {
			...
			if (checkedInSkuItems != null && checkedInSkuItems.size() > 0) {
				...
				barcodeFilePath = barcodeFilePath + "/" + "printBarcode_" + "grn_" + grn.getId() + "_All_" + StringUtils.substring(userWarehouse.getCity(), 0, 3)
						+ ".txt";
			}
		}
		try {

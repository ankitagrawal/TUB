...
public class SkuItemDaoImpl extends BaseDaoImpl implements SkuItemDao {
	...
	public SkuItem getSkuItem(SkuGroup skuGroup, SkuItemStatus skuItemStatus) {
     ...
     public SkuItem getSkuItemByBarcode(String barcode, Long warehouseId, Long statusId) {
        List<SkuItem> skuItems = getSession().createQuery("select si from SkuItem si where si.barcode = :barcode and si.skuGroup.sku.warehouse.id = :warehouseId and si.skuItemStatus.id = :statusId").
                setParameter("barcode", barcode).setParameter("warehouseId", warehouseId).setParameter("statusId", statusId).list();

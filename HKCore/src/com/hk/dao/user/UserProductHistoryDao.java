...
/**
 * DAO for domain model class UserProductHistory.
 * 
 * @author Hibernate Tools
 */
@Repository
public class UserProductHistoryDao extends BaseDaoImpl {
    ...
    @Transactional
    private UserProductHistory findByProductAndUser(Product product, User user) {
    ...
    public void updateIsAddedToCart(Product product, User user) {
        ...
        criteria.add(Restrictions.eq("user", user));
        List<UserProductHistory> userProductHistoryList = criteria.list();
        UserProductHistory userProductHistoryObject = userProductHistoryList == null || userProductHistoryList.isEmpty() ? null : userProductHistoryList.get(0);
        if (userProductHistoryObject != null) {
            userProductHistoryObject.setBought(true);
            save(userProductHistoryObject);
        }
    }
}

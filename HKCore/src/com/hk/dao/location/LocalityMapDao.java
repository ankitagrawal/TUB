...
@Repository
public class LocalityMapDao extends BaseDaoImpl {

  

  public LocalityMap findByAddress(Address address) {
    return (LocalityMap) getSession().createQuery("from LocalityMap l where l.address = :address")
        .setEntity("address", address)
        .uniqueResult();
  }
  ...
  public Double getDistanceInMeters(Double originLattitude, Double originLongitude, Double destLattitude, Double destLongitude) throws IOException {
    ...
    return 3956 * 2 * Math.asin(Math.sqrt(Math.pow(Math.sin((originLattitude - Math.abs(destLattitude)) * Math.PI / 180 / 2), 2) +
        Math.cos(originLattitude * Math.PI / 180) * Math.cos(Math.abs(destLattitude) * Math.PI / 180) * Math.pow(Math.sin((originLongitude - Math.abs(destLongitude)) * Math.PI / 180 / 2), 2)));
  }
...
public List<AddressDistanceDto> getClosestAddressList(Double originLattitude, Double originLongitude, Double distance, Integer lim, List<Address> addresses) {
    ...
    return getSession().createQuery(query)
        .setParameter("originLattitude", originLattitude)
        .setParameter("originLongitude", originLongitude)
        .setParameterList("addresses", addresses)
        .setParameter("distance", distance)
        .setMaxResults(lim).setResultTransformer(Transformers.aliasToBean(AddressDistanceDto.class))
        .list();
  }

  /* String query = "SELECT 3956 * 2 * ASIN(SQRT(POWER(SIN((:originLattitude - ABS(:destLattitude)) * 3.14287/180 / 2),2) +COS(:originLattitude * 3.14287/180 ) * COS(ABS(:destLattitude) * 3.14287/180) *POWER(SIN((:originLongitude - ABS(:destLongitude)) * 3.14287/180 / 2), 2) ))" +
  " FROM MapIndia mi";

   return (Double) getSession().createQuery(query)
  .setParameter("originLattitude", originLattitude)
  .setParameter("originLongitude", originLongitude)
  .setParameter("destLongitude", destLongitude)
  .setParameter("destLattitude", destLattitude)
  .list().get(0);*/
}
